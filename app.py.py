...
st.markdown("Explore o salário na área de Dados nos últi os anos. Utilize os filtros a esquerda para refinar sua análise.")
...
with col_graf1:
    if not df_filtrado.empty:
        ...
        grafico_cargos = px.bar(
            top_cargos, x='usd', y='cargo', orientation='h', title="Top 10 cargos por salário médio", labels={'usd': 'Média salarial anual (usd)', 'cargo': ''}
        )
        grafico_cargos.update_layout(
            title_x=0.1, yaxis={'categoryorder': 'total ascending'})
        st.plotly_chart(grafico_cargos, use_container_width=True)
    else:
        st.warning("Nenhum dado para exibir para gráfico de cargos.")
...
with col_graf2:
    if not df_filtrado.empty:
        ...
        st.warning("Nenhum dado para exibir para gráfico de distribuição.")
...
with col_graf4:
    if not df_filtrado.empty:
        ...
        media_df_pais = df_ds.groupby('residencia_iso3')[
            'usd'].mean().reset_index()
        grafico_paises = px.choropleth(media_df_pais, locations='residencia_iso3', color='usd', color_continuous_scale='rdylgn',
                                       title='Média salarial Data Scientist por país', labels={'usd': 'Salário médio (usd)', 'residencia_iso3': 'País'})
